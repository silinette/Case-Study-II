...
```{r, echo=FALSE, message = FALSE}
# Получаем таблицы с нагрузками
PCA_loadings <- as.data.frame(unclass(PCA1$loadings))
PAF_loadings <- as.data.frame(unclass(PAF1$loadings))

# Округляем значения
PCA_loadings <- round(PCA_loadings, 2)
PAF_loadings <- round(PAF_loadings, 2)

# Добавляем суффиксы
colnames(PCA_loadings) <- paste0(colnames(PCA_loadings), "_PCA")
colnames(PAF_loadings) <- paste0(colnames(PAF_loadings), "_PAF")

# Упорядочим столбцы попарно: RC1_PCA, RC1_PAF, RC2_PCA, RC2_PAF и т.д.
combined_list <- list()
for (i in 1:ncol(PCA_loadings)) {
  fac_name <- gsub("_PCA", "", colnames(PCA_loadings)[i])
  combined_list[[paste0(fac_name, "_PCA")]] <- PCA_loadings[[i]]
  combined_list[[paste0(fac_name, "_PAF")]] <- PAF_loadings[[i]]
}

# Собираем в датафрейм
Combined_Loadings <- as.data.frame(combined_list)
rownames(Combined_Loadings) <- rownames(PCA_loadings)

# Печатаем
print(Combined_Loadings)

# Для красивого отображения:
# DT::datatable(Combined_Loadings)
# или
# knitr::kable(Combined_Loadings, caption = "Side-by-Side Factor Loadings: PCA vs PAF")

# knitr::kable(Combined_Loadings, caption = "Comparison of PCA and PAF Loadings")
